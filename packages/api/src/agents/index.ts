...
export class InsightsEngine {
  ...
  async generateInsights(context: {
    ...
    try {
      ...
      // Use local insights generation (no AI dependency)
      const insights = this.generateLocalInsights(context);
  ...
  /**
   * Generate insights locally without AI dependency
   */
  private generateLocalInsights(context: {
    projectName: string;
    projectType: string;
    complexity: string;
    techStack: string[];
    ...
    if (context.complexity === "high") {
      insights.push({
        ...
        description:
          "High complexity projects benefit from microservices to manage different domains independently.",
        impact: "high",
        actionItems: [
          "Identify domain boundaries",
          "Plan service communication strategy",
          "Set up service discovery",
        ],
        relatedTechnologies: ["Docker", "Kubernetes", "API Gateway"],
    ...
    if (context.techStack.length > 0) {
      insights.push({
        id: `insight-scale-${Date.now()}`,
        type: "best_practice",
        title: "Implement Horizontal Scaling",
        description:
          "Design your application for horizontal scaling from the start.",
        impact: "high",
        actionItems: [
        ...
        relatedTechnologies: ["Redis", "Load Balancer", "CDN"],
        createdAt: new Date().toISOString(),
      });
    }
    ...
    insights.push({
      ...
      relatedTechnologies: ["OAuth", "JWT", "OWASP"],
      createdAt: new Date().toISOString(),
    });
